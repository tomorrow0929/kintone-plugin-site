...
| カテゴリ | 任意。入力欄の下に、いま使われている表記が出る。押すと入る |
...
**登録画面に並び順の入力欄は無い。** 一覧の「順」列の ↑↓ だけで変える。

- 新しく登録したもの（まとめて取り込みも含む）は、**必ず一番下**に入る
- ↑↓ を押すと、そのときの表示順のとおりに 1 から振り直して保存する。
  番号が重複していても、一度押せばきれいに並ぶ
- **公開サイトの表示順もこの値で決まる。**
  ただし反映には再デプロイが必要（静的HTMLはビルド時に作られるため）

番号を手で入れる方式をやめたのは、36本あると「どの番号が空いているか」を
管理できなくなるため。上下に動かすだけで済むほうが早い。
...
- [x] 並び順を一覧の ↑↓ で変える方式に変更（2026-09-15）
      登録画面の手入力をやめ、新規は一番下に入るようにした
- [x] 登録画面のカテゴリ欄に、いま使われている表記を出す（2026-09-15）
      表記ゆれ（「データ連携・インポート」と「〜系」）を見えるようにした
- [ ] カテゴリの表記ゆれをデータごと直す（管理画面の作業。直したら再デプロイ）
